...
async function fetch() {
  if (route.params.username == null) return;
  await findOne(route.params.username);
  rootStore.changeDomTitle({ title: user.username });
}

async function submit() {
  if (newPwd.value === checkPwd.value) {
    const updatedUser = purify(Object.assign(user, { newPwd }));
...
function del(username) {
  $Modal.confirm({
    ...
    title: t("oj.warning"),
    content: `<p>${t("oj.will_remove_user_continue_or_not", { username: username })}</p>`,
    onOk: async () => {
      await remove({ username: username });
      router.push({ name: "home" });
      $Message.success(t("oj.remove_user_success", { username: username }));
    },
    onCancel: () => {
  ...
  <div class="user-wrap">
    <Row>
      <Col :span="6">
        ...
        <Icon type="person" />&nbsp;&nbsp;{{ `${t("oj.nick")}: ${user.nickname}` }}
        <div v-if="group.length > 0" class="group">
          <Icon type="person-stalker" />&nbsp;&nbsp;{{ `${t("oj.group")}: ${group}` }}
        </div>
        <div v-if="user.signaure" class="motto">
          <Icon type="edit" />&nbsp;&nbsp;{{ `${t("oj.motto")}: ${user.signaure}` }}
        </div>
        <div v-if="user.email">
          <Icon type="email" />&nbsp;&nbsp;{{ `${t("oj.mail")}: ${user.email}` }}
        </div>
        ...
        <Button
          v-if="isAdmin && user.uid !== 'admin' && canRemove"
